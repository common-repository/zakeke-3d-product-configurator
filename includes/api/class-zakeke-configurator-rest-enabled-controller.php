 ...
 *
 * Handles requests to the /enabled endpoint.
 *
 * @category API
 ...
 */
class ZakekeConfigurator_REST_Enabled_Controller extends WC_REST_Controller {
	 ...
	 * @var string
	 */
	protected $namespace = 'zakeke-configurator/v1';
	...
	public function register_routes() {
			...
			array(
				...
				'args'                => array_merge( $this->get_endpoint_args_for_item_schema( WP_REST_Server::CREATABLE ),
					array(
						'id' => array(
							'type'        => 'integer',
							'description' => __( 'Product id of the product that became configurable.', 'zakeke-configurator' ),
							'required'    => true
						),
					) ),
			),
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/(?P<id>.*)', array(
			'args'   => array(
				'id' => array(
					'description' => __( 'Product id of the product that is no longer customizable.', 'zakeke-configurator' ),
					'type'        => 'integer',
					'required'    => true
				),
			),
	...
	public function create_item_permissions_check( $request ) {
		if ( current_user_can( 'manage_options' ) || current_user_can( 'manage_woocommerce' ) || current_user_can( ZakekeConfigurator::CAPABILITY ) ) {
			return true;
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'title'      => 'zakeke_configurator_enabled',
			'type'       => 'object',
			'properties' => array(
				'id' => array(
					'description' => __( 'Product id.', 'zakeke-configurator' ),
					'type'        => 'integer',
